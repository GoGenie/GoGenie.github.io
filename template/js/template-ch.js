...
$("#share").jsSocials({
  ...
  url: "http://gogenieapp.com/free-excel-template-for-employee-scheduling-ch.html",
    text: "Excel員工更表範本免費下載",
    showLabel: false,
    showCount: true,
    showCount: "inside",
    shareIn: "popup",
...
$( "#submit" ).click(function(e) {
  ...
  if (checkValidation(data.data)) {
    ...
    var url = "http://gogenie-mailer.herokuapp.com/template_download_ch";
    $.post(url, data, function(data, status){
      console.log("Data: " + data + "\nStatus: " + status);
      console.log(data);
    });
    setTimeout(function(){ $('#myModal').modal('toggle'); }, 500);
    setTimeout(function(){ window.location.href = "/free-excel-template-for-employee-scheduling-sent-ch.html"; }, 1000);
  };
});
